...
def render(state):
    ...
    agents_theta = state.entities.position.orientation[:max_agents][exists_agents][exists_agents]
    ...
    agents_colors_rgba = [colors.to_rgba(np.array(c), alpha=1.) for c in state.agents.color[exists_agents]]
    x_objects, y_objects = objects_pos[:, 0], objects_pos[:, 1]
    object_colors_rgba = [colors.to_rgba(np.array(c), alpha=1.) for c in state.objects.color[exists_objects]]
...
def render_history(state_history, pause=0.001, skip_frames=1):
    ...
    for t in range(0, len(state_history), skip_frames):
        ...
        exists_agents, exists_objects = entities.exists[:max_agents], entities.exists[max_agents:] 
        exists_agents = jnp.where(exists_agents != 0)
        exists_objects = jnp.where(exists_objects != 0)

        agents_pos = entities.position.center[:max_agents][exists_agents]
        agents_theta = entities.position.orientation[:max_agents][exists_agents][exists_agents]
        ...
        x_agents, y_agents = agents_pos[:, 0], agents_pos[:, 1]
        agents_colors_rgba = [colors.to_rgba(np.array(c), alpha=1.) for c in agents.color[exists_agents]]
        x_objects, y_objects = objects_pos[:, 0], objects_pos[:, 1]
        object_colors_rgba = [colors.to_rgba(np.array(c), alpha=1.) for c in objects.color[exists_objects]]
