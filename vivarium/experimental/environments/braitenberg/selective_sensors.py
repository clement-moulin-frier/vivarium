...
def sensor(dist, relative_theta, dist_max, cos_min, max_agents, senders, target_exists):
    jax.debug.print("Before sensor_fn")
    jax.debug.print("dist.shape = {x}", x=dist.shape)
    jax.debug.print("relative_theta.shape = {x}", x=relative_theta.shape)
    jax.debug.print("dist_max.shape = {x}", x=dist_max.shape)
    jax.debug.print("cos_min.shape = {x}", x=cos_min.shape)
    jax.debug.print("raw_proxs.target_exists = {x}", x=target_exists.shape)

    raw_proxs = sensor_fn(dist, relative_theta, dist_max, cos_min, target_exists)
    jax.debug.print("raw_proxs.shape = {x}", x=raw_proxs.shape)
...
def mask_sensors(state, agent_raw_proxs, ent_type_id, ent_target_idx):
    mask = jnp.where(state.entities.entity_type[ent_target_idx] == ent_type_id, 0, 1)
...
class SelectiveSensorsBraitenbergEnv(BaseEnv):
    ...
    def _step(self, state: State, neighbors: jnp.array, agents_neighs_idx: jnp.array, ag_idx_dense: jnp.array) -> Tuple[State, jnp.array]:
        ...
        ### Hardcoded behaviors for agents at the moment (aggr towards objects and fear towards agents)
        params_1 = behavior_to_params(Behaviors.AGGRESSION.value)
        params_2 = behavior_to_params(Behaviors.FEAR.value)
        sensed_1 = jnp.array([0, 1])
        sensed_2 = jnp.array([1, 0])
        params = jnp.array([params_1, params_2])
        sensed = jnp.array([sensed_1, sensed_2])
        
        # Do like if we had batches of params and sensed entities for all agents
        batch_params = jnp.tile(params[None], (self.max_agents, 1, 1 ,1))
        batch_sensed = jnp.tile(sensed[None], (self.max_agents, 1, 1))
        ###
